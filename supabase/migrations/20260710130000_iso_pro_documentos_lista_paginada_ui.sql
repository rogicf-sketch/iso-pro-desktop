-- Lista paginada com filtro de status (UI PC/web escala 11k+).
BEGIN;
...
BEGIN
  ...
  IF v_proj > 0 AND v_at >= v_proj - 1e-9 THEN
    RETURN 'atendido';
  END IF;
  IF v_at > 1e-9 THEN
    RETURN 'parcial';
...
BEGIN
  ...
  SELECT count(*)::int INTO v_total FROM filtrado;
...
-- IDs de todos os docs do filtro (seleção em massa sem carregar 11k na UI).
CREATE OR REPLACE FUNCTION public.iso_pro_list_documentos_planejamento_ids(
  p_tenant_id uuid,
  p_busca text DEFAULT NULL,
  p_status text DEFAULT NULL
)
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(to_jsonb(x.id) ORDER BY x.numero), '[]'::jsonb)
  INTO v_ids
  FROM (
    ...
    WHERE d.tenant_id = p_tenant_id
      ...
      AND (
        v_status IS NULL
        OR public.iso_pro_documento_status_derivado(p_tenant_id, d.id, d.status) = v_status
      )
    ORDER BY d.numero
  ) x;

  RETURN jsonb_build_object('ids', v_ids, '_source', 'tables');
END;
$$;

REVOKE ALL ON FUNCTION public.iso_pro_list_documentos_planejamento_ids(uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.iso_pro_list_documentos_planejamento_ids(uuid, text, text) TO anon, authenticated;
