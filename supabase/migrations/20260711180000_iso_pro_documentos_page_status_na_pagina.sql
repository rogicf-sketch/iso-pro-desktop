-- Lista paginada: calcular status_deriv SÓ na página (não nos 11k) quando filtro status = todos.
-- Com filtro de status, mantém derivação (mais lento, uso menos frequente).
...
BEGIN
  ...
  -- Caminho rápido: sem filtro de status → pagina primeiro, deriva só a página
  IF v_status IS NULL THEN
    ...
    contagem AS (SELECT count(*)::int AS total FROM matched),
    pagina AS (
      SELECT * FROM matched
      ORDER BY numero
      OFFSET v_off
  ...
  -- Com filtro de status: deriva antes de filtrar (mais lento)
  WITH base AS (
    SELECT
      d.id,
      d.numero,
...
COMMENT ON FUNCTION public.iso_pro_list_documentos_planejamento_page(uuid, text, int, int, text) IS
  'Lista paginada: status derivado só na página quando status=todos (escala 11k).';
